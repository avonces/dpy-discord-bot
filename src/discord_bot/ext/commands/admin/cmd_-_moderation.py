...
class Moderation(commands.Cog, name='Moderation', description='contains moderation-related commands'):
    ...
    @bridge.bridge_command(name='timeout', aliases=['mute'], description='timeouts/ mutes a server member')
    @commands.has_permissions(moderate_members=True)
    @commands.guild_only()
    async def timeout(self, ctx: bridge.BridgeContext, member: discord.Member,
                      days: int = 0, hours: int = 0, minutes: int = 15, seconds: int = 0, *, reason: str):
    ...
    @bridge.bridge_command(name='kick', description='kicks a server member')
    @commands.has_permissions(kick_members=True)
    @commands.guild_only()
    async def kick(self, ctx: bridge.BridgeContext, member: discord.Member, *, reason: str):
    ...
    @bridge.bridge_command(name='ban', description='bans a server member')
    @commands.has_permissions(ban_members=True)
    @commands.guild_only()
    async def ban(self, ctx: bridge.BridgeContext, member: discord.Member, *, reason: str):
    ...
    @bridge.bridge_command(name='unban', description='unbans a server member')
    @commands.has_permissions(ban_members=True)
    @commands.guild_only()
    async def unban(self, ctx: bridge.BridgeContext, member: discord.Member, *, reason: str):
